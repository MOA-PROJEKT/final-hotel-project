...
// gleiche Bilder wie Deluxe Doppelzimmer
...
export default function JuniorSuiteLarge() {
  useEffect(() => window.scrollTo(0, 0), []);

  const room = useMemo(() => rooms.find((r) => r.id === "junior-suite-large"), []);
  const images = useMemo(() => [z1, z2, z3, z4, z5, z6, z7, z8, z9], []);

  // ✅ Hero (änderbar)
  const DEFAULT_HERO_INDEX = 3; // z4

  const [active, setActive] = useState(DEFAULT_HERO_INDEX);
  const [open, setOpen] = useState(false);

  useEffect(() => {
    if (active >= images.length) setActive(DEFAULT_HERO_INDEX);
      ...
      <main className="min-h-screen bg-[#f7efe7] text-neutral-800 flex items-center justify-center px-4">
        <div className="max-w-xl text-center bg-white/70 border border-black/10 rounded-2xl p-8">
          ...
          <p className="text-neutral-600">
            Prüfe bitte, ob in <code>rooms.js</code> die ID <b>junior-suite-large</b> existiert.
          </p>
          <Link
            to="/rooms"
      ...
      <section className="relative">
        ...
        <div className="absolute bottom-6 left-0 right-0">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <h1 className="mt-2 text-white text-3xl sm:text-4xl font-semibold">{room.name}</h1>
            <p className="mt-2 text-white/90 max-w-2xl text-sm sm:text-base leading-relaxed">
              Unsere Empfehlung für alle, die etwas mehr Platz schätzen: eine großzügige Junior Suite
              mit Badewanne, Dusche und Blick nach Süden auf die Bergwelt.
            </p>
          </div>
        </div>
      </section>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white/70 border border-black/10 rounded-2xl p-6 h-[590px]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.35em] text-neutral-500">{room.category}</p>
              ...
              <div className="mt-6 border-t border-black/10 pt-6">
                ...
                <ul className="space-y-2 text-neutral-700">
                  <li className="flex gap-3"><span className="mt-1 h-2 w-2 rounded-full bg-[#c52b58]" />65 m² kombinierter Wohn- & Schlafbereich</li>
                  <li className="flex gap-3"><span className="mt-1 h-2 w-2 rounded-full bg-[#c52b58]" />Badewanne & Dusche</li>
                  <li className="flex gap-3"><span className="mt-1 h-2 w-2 rounded-full bg-[#c52b58]" />Blick nach Süden</li>
                  <li className="flex gap-3"><span className="mt-1 h-2 w-2 rounded-full bg-[#c52b58]" />Großzügiger Wohnkomfort</li>
                </ul>
